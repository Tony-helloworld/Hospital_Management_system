...
@Component
public class PPatientPrescriptionDao
{
	@Autowired
	SessionFactory sf;
	@Autowired
	...
	@Transactional
	public List<String[]> getPrescriptionList(String userID)
	{
		infoLog.logActivities("in PatientPrescriptionDao-getPrescriptionList: ");
		
		Session session= sf.getCurrentSession();
		Query q1= session.createQuery("from Opd where pid= :p AND status= :s");	//HQL use classname not tablename
		q1.setParameter("p", userID);
		q1.setParameter("s", 2);
